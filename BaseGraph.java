...
public class BaseGraph<NodeType, EdgeType extends Number> {
    ...
    //_kv_ public MapADT<NodeType, Node> nodes = null;
...
/*_kcole9_*/
    /**
     * Returns a list of all node data (keys) in the graph (map)
     *
     * @param 
     * @return the list of all node data in the graph
     */
    public List<NodeType> keySet(){
    	List<NodeType> keys = new LinkedList<NodeType>(); //type of list should not matter too much here
	for (NodeType data : nodes.keySet()){
		keys.add(data);
	}
	return keys;
    }
